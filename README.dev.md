# `yaml2nuctrans` developer documentation
...
# install yaml2nuctrans as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `yaml2nuctrans`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d yaml2nuctrans.XXXXXX)
git clone git@github.com:CQ-QianChen/yaml2nuctrans .
...
Visit
[https://test.pypi.org/project/yaml2nuctrans](https://test.pypi.org/project/yaml2nuctrans)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d yaml2nuctrans-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple yaml2nuctrans
```
...
Don't forget to also make a [release on GitHub](https://github.com/CQ-QianChen/yaml2nuctrans/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.